...
postgres:
  ...
  ports:
    - "5432:5432"
```
with a top-level `pgdata` named volume. The `dev` and `prod` services get `depends_on: [postgres]`
and a `DATABASE_URL` pointing at `postgres://simple_ai_os:simple_ai_os@postgres:5432/simple-ai-os`.
Local (non-Docker) `npm run dev` continues to work as long as `DATABASE_URL` is set in `.env`
(e.g. pointing at `localhost:5432` if Postgres is run separately).
